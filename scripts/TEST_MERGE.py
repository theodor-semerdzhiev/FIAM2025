...
# !! TEMP_NOTE: You can point this at your *original* text data folder
# (e.g., r'C:\_Files\School\Competitions\FIAM2025\data\global_text_data')
# to test the merge logic right now!
...
OUTPUT_FILE_PATH = r'C:\_Files\School\Competitions\FIAM2025\data\processed_data\final_merged_data.pkl'
...
COUNTRY_TO_FIC_MAP = {
    ...
    'Israel': 'ISL',
    'Japan': 'JPN',
    'South Korea': 'KOR', # <-- ADDED THIS FIX
    'United Kingdom': 'GBR',
    'Luxembourg': 'LUX',
    'Mexico': 'MEX',
    'Netherlands': 'NLD',
...
def main():
    ...
    df_embeddings = df_embeddings.dropna(subset=['normalized_name', 'fic', 'period_end_date'])
    ...
    # This is the "fuzzy" merge.
    # It links our filings to gvkey and iid.
    # 
    
    print("\n--- MERGE 1: Linking Embeddings to GVKEY/IID ---")
    
    # This is the 1-to-many merge.
    # One filing (e.g., 'VOLVO CAR') will match *all* its securities
    # (e.g., 'VOLVO CAR' + 'iid 01W' and 'VOLVO CAR' + 'iid 02W').
    ...
    # This is the final, precise merge.
    # We link on the *exact* identifiers:
    # 1. gvkey (company)
    # 2. iid (security)
    # 3. period_end_date (from filing) == char_eom (from returns)
    # 
    ...
    print("\n--- MERGING COMPLETE ---")
    print(f"Total rows in original embeddings: {len(df_embeddings)}")
    print(f"Total rows successfully matched to a gvkey/iid: {len(df_linked_filings)}")
    print(f"Total final rows linked to a T+1 return: {len(df_final_data)}")

    if df_final_data.empty:
        print("WARNING: Final dataset is empty. Check your merge keys and dates.")
        print("Common issues:")
        print("- Date mismatch (e.g., 2023-09-30 vs 2023-09-01)")
