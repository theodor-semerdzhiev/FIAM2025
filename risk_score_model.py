...
if __name__ == "__main__":
    ...
    cik_link_path = os.path.join(data_dir, 'cik_gvkey_linktable.csv')
    embeddings_batch_dir = 'embedding_batches'

    # --- Step 1: Load All Pre-processed Data ---
    print("--- Loading Quantitative Data (for target variable and IDs) ---")
    df_quant = pd.read_csv(ret_sample_path, low_memory=False)
    ...
    # --- Step 2: Merge Datasets to Create the Master DataFrame ---
    ...
    # --- UPDATED: Changed from 'left' to 'inner' merge ---
    # This will only keep rows that have BOTH quantitative and text data.
    ...
    # No longer need to fillna for embeddings, as the inner merge ensures they exist.
    ...
    # --- NEW: Step 3: Create the Downside Risk Target Variable ---
    # This is our proxy for optimizing for Sortino. We want the model to predict
    # the squared negative returns. A high prediction means high downside risk.
    ret_var = "stock_ret"
    df_master['downside_risk_target'] = np.where(df_master[ret_var] < 0, df_master[ret_var]**2, 0)
    
    # --- Step 4: Train Model using Expanding Window ---
    ...
    # OOS of 5 years
    while (starting + pd.DateOffset(years=11 + counter)) <= pd.to_datetime("20200101", format="%Y%m%d"):
        ...
        # The model's prediction is now the risk score.
    ...
    print("\nThis score, trained only on the text narrative, can now be used as a feature in a higher-level model.")
