...
# ---- NEW: Market-cap-based trading fees (per turnover) ----
# "me" is in millions. Threshold: >= 15,000 (i.e., $15B) is Large; below is Small.
LARGE_CAP_ME_MILLIONS = 15_000.0
FEE_PER_TURN_LARGE    = 0.0025   # 0.25% per turnover for large caps
FEE_PER_TURN_SMALL    = 0.0100   # 1.00% per turnover for small caps
...
# NEW: include "me" (market cap in millions)
proxy_cols = list(
    set(
        ["id", "char_year", "char_month", "iid", "excntry", "me"]
        + list(LIQ_COLS)
...
def build_long_short(pred_like: pd.DataFrame, label: str):
    ...
    for (y, m), dfm in groups:
        ...
        # --- NEW: tag cap bucket for tracking (based on 'me' in millions) ---
    ...
    # --- NEW: market-cap composition tracking per side ---
    def _cap_counts(df_side):
        if df_side.empty:
            return pd.DataFrame(columns=["year","month","n_large","n_small"])
        tmp = df_side.copy()
        # treat unknown me as small for counting fees conservatively
    ...
    # --- NEW: apply market-cap-based fees per turnover (tracked by composition) ---
    # Effective fee rate per side = fraction_large * fee_large + fraction_small * fee_small
    ...
    # Fees deducted from portfolio return (per side), scaled by each side's exposure weight
    ...
    # Keep legacy total too (based on raw counts)
...
for label, out in ls_outputs.items():
    ...
    if not monthly_src.empty:
        ...
        # --- NEW: average cap composition (tracking) ---
        ...
        # --- NEW: average fees paid (as monthly return drag) ---
...
print(" - summary_stats_by_variant.csv        <-- one row per variant (BASE, LIQ_ONLY, ILLIQ_ONLY) + avg_*_turnover, avg cap mix, avg fee drag")
